...
# Sequential Muon-OGD SFT pipeline for Qwen2.5-1.5B-Instruct
# Task order: Coding -> Math -> Medical
# All artifacts are saved under: 1.5B-instruct/

ROOT_DIR="1.5B-instruct"
LOG_ROOT="${ROOT_DIR}/logs_seq2_muon"
...
python -u train_coding_bigcodebench_muon_ogd.py \
  ...
  --split complete \
  --output_dir "${OUT_ROOT}/sft_seq2_coding_qwen1.5b_instruct_muon_ogd" \
  --max_length 2048 \
  --batch_size 1 \
  --grad_accum 8 \
  --epochs 1 \
...
python -u train_math_sft_svd.py \
  --model_id "${OUT_ROOT}/sft_seq2_coding_qwen1.5b_instruct_muon_ogd" \
  --ci_model_id "${OUT_ROOT}/sft_seq2_coding_qwen1.5b_instruct_muon_ogd" \
  --output_dir "${OUT_ROOT}/sft_seq2_math_qwen1.5b_instruct_muon_ogd_from_coding" \
...
python -u train_medical_muon_ogd.py \
  ...
  --output_dir "${OUT_ROOT}/sft_seq2_medical_qwen1.5b_instruct_muon_ogd_from_coding_math" \
  --max_length 1536 \
  --batch_size 1 \
  --grad_accum 8 \
  --epochs 1 \
...
echo "1.5B-Instruct Muon-OGD SFT pipeline completed successfully."
