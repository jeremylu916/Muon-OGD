...
# Qwen2.5-1.5B-Instruct runner with Sculpting-Subspace training:
# - Subspace runs: 2 repeats
# - Same stage/task/eval process as qwen1p5b_O-LoRA.sh
...
# Sculpting-Subspace hyperparameters (override via env vars)
...
preflight_subspace_checks() {
  ...
  for f in "${scripts[@]}"; do
    ...
    if ! grep -q -- "--use_sculpt_subspace" "${f}"; then
      echo "[ERROR] ${f} does not expose --use_sculpt_subspace" >&2
      exit 1
    fi
  done
  echo "[preflight] subspace support checks passed"
}
...
run_subspace() {
  ...
  python -u train_coding_bigcodebench_sft.py \
    ...
    --split complete \
    --output_dir "${out_root}/sft_seq2_coding_qwen1.5b_instruct_subspace" \
    --max_length 2048 \
    --lr 5e-6 \
    --batch_size 1 \
    --grad_accum 8 \
  ...
  echo "[Subspace][Run ${run_idx}] Stage A eval"
  eval_stage \
    "${out_root}/sft_seq2_coding_qwen1.5b_instruct_subspace" \
    "${res_root}" \
    "${log_root}" \
    "stage_a" \
    "bcb_hard_subspace"
  ...
  python -u train_math_sft.py \
    --model_id "${out_root}/sft_seq2_coding_qwen1.5b_instruct_subspace" \
    --output_dir "${out_root}/sft_seq2_math_qwen1.5b_instruct_subspace_from_coding" \
  ...
  echo "[Subspace][Run ${run_idx}] Stage B eval"
  eval_stage \
    "${out_root}/sft_seq2_math_qwen1.5b_instruct_subspace_from_coding" \
    "${res_root}" \
    "${log_root}" \
    "stage_b" \
    "bcb_hard_subspace_from_coding_math"

  echo "[Subspace][Run ${run_idx}] Stage C train"
  python -u train_medical_sft.py \
    --model_id "${out_root}/sft_seq2_math_qwen1.5b_instruct_subspace_from_coding" \
    --output_dir "${out_root}/sft_seq2_medical_qwen1.5b_instruct_subspace_from_coding_math" \
  ...
  echo "[Subspace][Run ${run_idx}] Stage C eval"
  eval_stage \
    "${out_root}/sft_seq2_medical_qwen1.5b_instruct_subspace_from_coding_math" \
    "${res_root}" \
    "${log_root}" \
    "stage_c" \
    "bcb_hard_subspace_final"
}
    ...
    for s in stages:
        ...
        for r in runs:
            ...
            coding = load_metric(os.path.join(rr, "coding", {
                "stage_a": "bcb_hard_subspace",
                "stage_b": "bcb_hard_subspace_from_coding_math",
                "stage_c": "bcb_hard_subspace_final",
            }[s], "pass_at_k.json"))
...
out_path = os.path.join(root, "summary_mean_std_subspace.json")
